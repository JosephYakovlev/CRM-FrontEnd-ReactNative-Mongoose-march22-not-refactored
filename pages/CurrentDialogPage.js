...
export default function CurrentDialogPage(dialogdata) {
    ...
    const routeToRunningContractHandler = async () => {
        ...
        const casedata = await  axios.get(`http://62.113.97.220:8800/api/openedCases/${thisDialog.companion}`).then((casedata)=>{
            ...
            navigation.navigate('OpenedCasePage', casedata.data)
    ...
    const renderMessage = (item,index) => {
        ...
        return (
            <SafeAreaView style={styles.container}>
                ...
                <View style={{flex: 1, marginTop: thisDialog.type === "PERSONAL MESSAGE" ? 75+headerHeight : 105 + headerHeight, marginBottom: imageWidth+8, 
                    backgroundColor: thisDialog.type === "PERSONAL MESSAGE" && 'lightblue'  || 
                                     thisDialog.type === "RUNNING CONTRACT" && '#9DFFCB' || 
                                     thisDialog.type === "CONCLUDING CONTRACT" && '#F0FF9D',
                ...
                <View style={{top: 0, position: 'absolute'}}>    
                    {thisDialog.type === "PERSONAL MESSAGE" && 
                        <TouchableOpacity onPress={()=> thisDialog.companion != "BUILDER" && routeToUserHandler(thisDialog.companion)} style={{width:'100%', height: 75, flexDirection: 'row', backgroundColor: 'white', borderBottomWidth: 1, marginTop: headerHeight}}>
                ...
                <View style={{position:'absolute',bottom: 0, backgroundColor: 'white', width: '100%', paddingBottom: 5, borderTopWidth: 1}}>
                    <View style={{width: '100%', minHeight: imageWidth+8, maxHeight: 100, flexDirection: 'row'}}>
                        ...
                        <TouchableOpacity onPress={() => {
                            thisDialog.type === "PERSONAL MESSAGE" && thisDialog.companion != "BUILDER" && sendNewMessageHandler()
                            thisDialog.type === "CONCLUDING CONTRACT" && sendNewConclCMessageHandler()
                            thisDialog.type === "RUNNING CONTRACT" && sendNewRunningMessageHandler()
                            thisDialog.type === "PERSONAL MESSAGE" && thisDialog.companion == "BUILDER" && Alert.alert(
                                "Мы приносим извинения...",
                                "Обратная связь пока в разработке",
                                [
                                  { text: "OK", onPress: () => console.log("OK Pressed") }
                                ]
                              );
                        }} style={{width: '15%', justifyContent: 'flex-end', alignItems: 'center'}}>
